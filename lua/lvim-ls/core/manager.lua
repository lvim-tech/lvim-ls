-- lvim-ls: low-level LSP lifecycle manager.
...
local function build_efm_lsp_config(root_dir)
    ...
    local project_mod = root_dir and project or nil
    local filetypes = {}
    local languages = {}
...
--- Is dependency `dep` missing (not installed)?
--- Delegates to lvim-pkg (the single source of truth, transition-aware) when
--- available; otherwise falls back to the original Mason bin-dir check.
---@param dep string
---@return boolean
local function is_missing(dep)
    ...
    if ok then
        return not pkg.is_installed("mason", dep)
    end
...
---@param server_name string
---@return string[]  Missing dependency names (Mason package names)
...
M.ensure_lsp_for_buffer = function(server_name, bufnr)
    ...
    if server_name ~= "efm" then
        local missing = M.missing_tools_for_server(server_name)
        if #missing > 0 then
...
M._start_server_for_buffer = function(server_name, bufnr, mod)
    ...
    if project.is_server_disabled(root_dir, server_name) then
    ...
    local proj_data = project.load_server(root_dir, server_name)
